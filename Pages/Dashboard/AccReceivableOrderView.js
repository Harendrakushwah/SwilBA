...
const AccReceivableOrderView = ({route}) => {
  ...
  const fullReportData = useSelector(state => state?.commonSlice?.Accounts?.fullReportData);
  ...
  useEffect(() => {
    setIsLoading(true);
    dispatch(getSingleOutstandingReport(data, autoAdjust)).then((response)=>{
      ...
      {!isLoading ? (
        <>
          <View style={styles.titleView}>
            <Text style={styles.titleText}>Receivables Report</Text>
            <View
              style={{
                flexDirection: 'row',
                alignItems: 'center',
...
export default AccReceivableOrderView
